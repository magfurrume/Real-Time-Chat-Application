...
export default function ChatLayout() {
  ...
  const handleAcceptCall = () => {
    if (incomingCall) {
      answerCall(incomingCall.callerId, incomingCall.offer)
      ...
      <div className="flex-1 min-w-0 flex flex-col z-10"> {/* Ensure chat window stacks properly */}
        <ChatWindow
          ...
          onOpenSidebar={handleOpenSidebar} // Pass function to open sidebar
           onStartCall={() => startCall(selectedFriend?.id)}
        />
      </div>
      ...
      {incomingCall && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded shadow-lg text-center">
            <p className="mb-4">Incoming Call from User {incomingCall.callerId}</p>
            <button className="bg-green-500 text-white px-4 py-2 mr-4 rounded" onClick={handleAcceptCall}>
              Accept
            </button>
            <button className="bg-red-500 text-white px-4 py-2 rounded" onClick={handleRejectCall}>
              Reject
            </button>
            <audio src="/ringtone.mp3" autoPlay loop />
          </div>
        </div>
      )}
